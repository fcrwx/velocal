...
#		$fh = new IO::File "> debug.txt" unless $debug;
		...
		print { $debug ? *STDOUT : $fh } "<br>\n";
		print { $debug ? *STDOUT : $fh } "<br>\n";
		print { $debug ? *STDOUT : $fh } "<hr>\n";
		print { $debug ? *STDOUT : $fh } "Announcements:<br>\n";
		print { $debug ? *STDOUT : $fh } "<ul>\n";

		my $announcements = read_file('/var/www/announcements.html');
		print { $debug ? *STDOUT : $fh } "$announcements\n";

#		print { $debug ? *STDOUT : $fh } "<li><a href='http://www.kansascyclist.com/podcast'>Kansas Cyclist Podcast</a>: Covering Bicycling News and Information from Arkansas, Colorado, Iowa, Kansas, Missouri, Nebraska, and Oklahoma\n\n";
		print { $debug ? *STDOUT : $fh } "</ul>\n";
