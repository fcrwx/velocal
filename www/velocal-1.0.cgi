...
# calling Mktime: year:2006 month:4 day:4 hour:38 minutes:26 seconds:17
print STDERR "calling Mktime: year:$year month:$month day:$day hour:$hour minutes:$minutes seconds:$seconds\n";
	my $time = Mktime($year,$month,$day,$hour,$minutes,$seconds);
#push @messages, "mktime: $year/$month/$day $hour:$minutes:$seconds";
	my ($gm_year,$gm_month,$gm_day,$gm_hour,$gm_minutes,$gm_seconds) = Gmtime($time);
#push @messages, "gmtime: $gm_year/$gm_month/$gm_day $gm_hour:$gm_minutes:$gm_seconds";
...
#push @messages, "timestring: $timestring";
		return ($timestring);
	}
	else {
...
#push @messages, "main: local date is $year/$month/$day";
...
#	$cal->todaycellclass('today');
	...
	my $select = "select name_first,name_last,email from users where user_id = $user_id";
	my $qh = $dbh->prepare($select);
	$qh->execute();
	my ($name_first,$name_last,$email) = $qh->fetchrow_array();
	...
	if ($logged_in) {
		...
		my $qh = $dbh->prepare($select);
		$qh->execute();
		my ($name_last,$name_first,$biography,$pref_pace,$pref_terrain,$photo_filename,$db_email) = $qh->fetchrow_array();
	...
	if ($logged_in) {
		...
		my $qh = $dbh->prepare($select);
		$qh->execute();
		my ($name_last,$name_first,$biography,$pref_pace,$pref_terrain,$photo_filename) = $qh->fetchrow_array();
		...
		print
			...
			Tr(
				...
				td(
					popup_menu({-name=>'pref_terrain',-values=>\@terrain_values,-default=>$pref_terrain,-override=>1}),
		...
		if ($everything_ok) {
			my $update = "
				...
				where
					user_id = $user_id";
			my $qh = $dbh->prepare($update);
			$qh->execute($name_last,$name_first,$biography,$pref_pace,$pref_terrain);
	...
	if ($logged_in) {
		...
		if (($type eq 'public') || ($type eq 'restricted')) {
			if ($type eq 'public') {
				...
				$select = "select name_first,name_last,email from users where user_id = $user_id";
				$qh = $dbh->prepare($select);
				$qh->execute();
				my ($name_first,$name_last,$email) = $qh->fetchrow_array();
	...
	if ($logged_in) {
		...
		if ($authorized) {
			...
			my ($group_name,$group_id,$title,$start_time,$location,$pace,$terrain,$distance,$notes,$event_creator) = $qh->fetchrow_array();
				...
				blockquote(
					...
					table(
						...
						Tr(
							...
							td(
								textarea({-name=>'comments',-value=>$comments,-cols=>$comments_columns,-rows=>$comments_rows,-style=>'overflow: visible;',-onKeyDown=>"expandTextArea(this, event, $comments_columns);",-override=>1})
							)
						)
					),
					br(),
	...
	if ($logged_in) {
		...
		if ($authorized) {
			...
			my $qh = $dbh->prepare($select);
			$qh->execute();
			my ($name,$city,$state,$description,$type,$default_pace,$default_terrain,$default_start_location,$default_start_time) = $qh->fetchrow_array();
			...
			while (my ($name_first,$name_last,$user_id,$email,$pref_pace,$pref_terrain,$photo) = $qh->fetchrow_array()) {
				$group_total_members++;
	...
	if ($logged_in) {
		...
		if ($count != 0) {
			...
			if ($count != 0) {
				...
				$select = "select name_first,name_last,email from users where user_id = $user_id_action";
				$qh = $dbh->prepare($select);
				$qh->execute();
				my ($name_first,$name_last,$email) = $qh->fetchrow_array();
...
# decided, don't notify people who are waiting approval, just delete the group
#			# notify users in the approval queue here
#			$select = "
#				select 
#					u.email,
#					u.name_first,
...
#				open(MAIL,"| $sendmail -t");
#				print MAIL "To: $name_first $name_last <$email>\n";
#				print MAIL "From: $registration_name <$registration_email>\n";
#				print MAIL "Subject: VeloCal Group Deleted\n";
#				print MAIL "Content-type: text/html\n\n";
#				print MAIL "You are currently in the approval queue for the group '$name'. Unfortunately this group has just been deleted. Therefore, you have been removed from the approval queue. No further action is required on your part. Thank you.";
#				close(MAIL);
#			}
...
# TODO: this SQL needs to be fixed
	...
	if ($logged_in) {
		...
		if ($edit eq 'Y') {
			...
			($city,$state,$description,$type,$name,$default_pace,$default_terrain,$default_start_location,$default_start_time) = $qh->fetchrow_array();
				...
				if ($edit eq 'Y') {
					...
					my $update = "
						...
						where
							group_id = $group_id";
					my $qh = $dbh->prepare($update);
					$qh->execute($city_name,$description,$name,$default_start_location);
...
#		my ($day,$month,$year) = (localtime(time))[3..5];
		...
		$month += 1;
		($year,$month,$day) = split ':', &$convert_to_localtime("$year:$month:$day:$hour:$min:$sec");
#push @messages, "add: local date is $year/$month/$day";
...
#push @messages, "check add: localtime is $now_year/$now_month/$now_day $now_hour:$now_min:$now_sec";
		...
		if (($check eq 'true') && ($data_valid == 1)) {
			...
			$select = "select name_last,name_first,email from users where user_id = $user_id";
			$qh = $dbh->prepare($select);
			$qh->execute();
			my ($name_last,$name_first,$email) = $qh->fetchrow_array();
