...
while (my ($group_id,$email,$event_id,$reminder_hours,$send_reminder,$now,$title,$start_time,$location,$pace,$terrain,$distance,$notes,$name,$status,$comments,$user_id) = $qh->fetchrow_array()) {
