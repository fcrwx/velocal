...
my $domain_correct_name = 'VeloCal Support';
my $domain_correct_email = 'velocal@nicola.textdrive.com';
...
my $kansasCityLink = "http://www.kansascyclist.com/podcast";
###
...
my $get_rss_key = sub {
	my $pass = new String::Random;
	my $rss_key = $pass->randpattern("nnCncCnnccccCCCncCnnCCccnncnCCnc");
	return($rss_key);
};
...
END
	...
	my $stackhouse = a({-href=>"http://www.stackhousemortgage.com"},img({-src=>'stackhouse.gif',-border=>0}));
	...
	if ($logged_in) {
		my $user_id = $q->cookie('VeloCalUserID');
		my $select = "select name_first,rss_key from users where user_id = $user_id";
		my $qh = $dbh->prepare($select);
		$qh->execute();
		my ($username,$rss_key) = $qh->fetchrow_array();

		if ($rss_key eq '') {
			$rss_key = &$get_rss_key();
			my $update = "update users set rss_key='$rss_key' where user_id = $user_id";
		...
		print
			...
			start_html(
				...
				-leftmargin => 0,
				-topmargin => 0,
				-style => {-src=>'velocal-1.7.css'},
				-head => Link({
									-rel=>'alternate',
									-title=>'VeloCal RSS',
									-href=>"$script?a=rss&u=$user_id&r=$rss_key",
...
#		my $fundraising_met = 40;
#		my $fundraising_goal = 215;
#		my $fundraising_percentage = int(($fundraising_met/$fundraising_goal)*100);
#		my $fundraising_remaining = $fundraising_goal - $fundraising_met;
#
#		$fundraising_met = "\$$fundraising_met";
#		$fundraising_remaining = "\$$fundraising_remaining";	
#
#		print 
#			start_div({-style=>"margin: 5px 5px 0px 5px; padding: 5px 5px 5px 5px; border: 1px solid #7d7a76; background-color: #beb7ae;"}),
#			start_table({-cellpadding=>5,-cellspacing=>0,-width=>"100%"}),
#			start_Tr(),
#			td(
#				{-width=>"250",-valign=>"top"},
#				"It takes \$215/year to keep VeloCal going. Please consider helping us for 2009!"
#			),
#			td(
#				{-align=>"center",-valign=>"top"},
#
#				div(
#					{-style=>"border: 1px solid #7d7a76; padding: 1px;"},
#					table(
#						{-width=>"100%",-cellpadding=>7,-cellspacing=>0},
#						Tr(
#							td(
#								{-width=>"$fundraising_percentage%",-bgcolor=>'#fe2e2e',-valign=>"center",-align=>"center"},
#								font({-color=>'white'},b($fundraising_met))
#							),
#							td(
#								{-bgcolor=>'#d0d0d0',-valign=>"center",-align=>"center"},
#								$fundraising_remaining
#							)
#						)
#					)
#				)
#			),
#			start_td({-align=>"center",-valign=>"top",-width=>75});
#
#
#		print<<__END__;
...
#		end_div();
		...
		foreach my $group_id (sort { $group_user{$a} cmp $group_user{$b} } keys %group_user) {
			...
			if ($homepage ne '') {
				print
					li(a({-href=>"http://$homepage"},'Homepage'));
			}
		...
		print<<__END__;
<div style="padding:5px 10px 5px 10px;">It takes \$215 per year to keep VeloCal going. Please consider helping out!</div>
<form action="https://www.paypal.com/cgi-bin/webscr" method="post">
<input type="hidden" name="cmd" value="_s-xclick">
<input type="image" src="https://www.paypal.com/en_US/i/btn/x-click-but04.gif" border="0" name="submit" alt="Make payments with PayPal - it's fast, free and secure!">
<input type="hidden" name="encrypted" value="-----BEGIN PKCS7----------END PKCS7-----
...
__END__
		...
		print
			start_table({-width=>'100%',-cellpadding=>0,-cellspacing=>0,-border=>0}),
			start_Tr(),
			start_td({-valign=>'top',-colspan=>2});

		print
			div(
				{-id=>'PageHeader',-style=>'padding: 0px; margin: 5px 2px 0px 5px;'},
				table(
					{-class=>'headerTable',-width=>'100%',-cellpadding=>0,-cellspacing=>0},
					Tr(
						td(
...
my $page_end = sub {
		...
		td(
			...
			div(
				...
				a(
					{-href=>"$kansasCityLink"},
					img({
						-src=>"images/KansasCyclistPodcastLogo100.png",
						-border=>0,
						-id=>'rightAdImage'
					})
				),
				p(
					{-id=>'rightAdP'},
					a(
						{-href=>"$kansasCityLink",-id=>'rightAdLink'},
						'Covering Bicycling News and Information from Arkansas, ',
						'Colorado, Iowa, Kansas, Missouri, Nebraska, and Oklahoma.'
					)
				)
			)
		);
	...
	print
		...
		div(
			...
			" | ",
			"&copy; ", a({-href=>'http://karlandamy.com'},'Karl Olson')
		);
...
if ($action eq 'welcome') {
	...
	if (defined $password) {
		...
		print MAIL "From: $domain_correct_name <$domain_correct_email>\n";
		print MAIL "Reply-To: $registration_name <$registration_email>\n";
...
#       if found, go back to unregister, and set 'g' to list of group_ids
...
elsif (($action eq 'ical') || ($action eq 'rss')) {
	my $user_id = $q->param('u');
	my $rss_key = $q->param('r');

	# get RSS_GUID for user
	# if not match, stop
	# else continue
	my $select = "select user_id from users where user_id = $user_id and rss_key = '$rss_key'";
	my $qh = $dbh->prepare($select);
	$qh->execute();
	my ($valid_combo) = $qh->fetchrow_array();
	...
	my $next_next_month = $next_month + 1;
	$next_next_month = 1 if ($next_month == 12);
	my $next_next_year = $next_year;
	$next_next_year = ($next_year + 1) if ($next_month == 12);

	# pull all rides from the database

	$month = "0$month" if ($month < 10);
	$next_month = "0$next_month" if ($next_month < 10);
	$next_next_month = "0$next_next_month" if ($next_next_month < 10);
	$current_day = "0$current_day" if ($current_day < 10);
	my $month_start = "$year$month$current_day" . "000000";
	my $month_end = "$next_next_year$next_next_month" . "01000000";
	...
	my $current_timestamp = `date +"%a, %d %b %Y %T %Z"`;
	...
	if ($action eq 'rss') {
		print
			...
			' <channel>' . "\n",
			'  <title>VeloCal Ride Calendar</title>' . "\n",
			'  <link>http://velocal.org/</link>' . "\n",
			'  <description>Cycling Event Scheduler</description>' . "\n",
			'  <language>en-us</language>' . "\n",
			"  <lastBuildDate>$current_timestamp</lastBuildDate>" . "\n",
			'  <docs>http://blogs.law.harvard.edu/tech/rss</docs>' . "\n",
			'  <generator>vim/perl</generator>' . "\n",
			'  <managingEditor>karl.olson@gmail.com</managingEditor>' . "\n",
			'  <webMaster>karl.olson@gmail.com</webMaster>' . "\n",
			' ' . "\n";
	}
	elsif ($action eq 'ical') {
		print "Content-type: text/plain\n\n";
		print "BEGIN:VCALENDAR\r\n";
		print "VERSION:2.0\r\n";
		print "PRODID:-//VeloCal//VeloCal Ride Calendar v$version//EN\r\n";
	}
		...
		if ($start_time =~ /^(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})$/) {
			...
			if ($action eq 'rss') {
				print
					...
					"   <link>http://velocal.org/?a=event&amp;e=$event_id</link>" . "\n",
					"   <guid isPermaLink=\"true\">http://velocal.org/?a=event&amp;e=$event_id</guid>" . "\n",
					"   <description><![CDATA[" . "\n",
					"     Group Name: $name" . "<br />" . "\n",
					"     Event: $title" . "<br />" . "\n",
					"     Your Status: $status" . "<br />" . "\n",
					"     Event Date: $event_year-$event_month-$event_day" . "<br />" . "\n",
					"     Event Time: $event_hour:$event_minute" . "<br />" . "\n",
			...
			# iCalendar data files are ASCII plain text files with each 
			# line terminated by CRLF (in hex: 0D0A). Lines should be 
			# limited to 75 octets (not characters) long. Where a data 
			# item is too long to fit on a single line it can be continued 
			# on following lines by starting the continuation lines with 
			# a space character (in hex: 20) or a tab character (in hex: 09). 
			# If a data item needs to contain a linefeed this is encoded 
			# as \n (in hex: 5C6E).
				...
				$notes =~ s/\n/\r\n /g;
				print "BEGIN:VEVENT\r\n";
				print "DTSTART:$event_year$event_month$event_day" . "T" . "$event_hour$event_minute" . "00\r\n";
				print "DTEND:$event_year$event_month$event_day" . "T" . "$event_hour$event_minute" . "00\r\n";
				print "SUMMARY:$name :: $title\r\n";
	...
	my $select = "select mon_cal,rss_key from users where user_id = $user_id";
	my $qh = $dbh->prepare($select);
	$qh->execute();
	my ($mon_cal,$rss_key) = $qh->fetchrow_array();
	...
	$select = "
		...
		from
			...
			groups g
		where
			e.group_id in (select group_id from group_user where user_id = $user_id) and
			e.event_id = u.event_id and
			e.group_id = g.group_id and
			u.user_id = $user_id and
			e.start_time < '$month_end' and
		...
		table(
			...
			Tr(
				...
				td(
					...
					a(
						{-href=>"$script?a=rss&u=$user_id&r=$rss_key"},
						img({-src=>'images/rss.png',-border=>0,-style=>'padding: 0px 2px 0px 0px;'})
					),
					a(
						{-href=>"$script?a=ical&u=$user_id&r=$rss_key"},
						img({-src=>'images/ical.png',-border=>0,-style=>'padding: 0px 6px 0px 0px;'})
					)
				)
			)
		),
	...
	if ($logged_in) {
		...
		if ($authorized) {
			...
			$homepage =~ s/^http:\/\///g;
			$homepage =~ s/\s//g;
			($homepage eq '') ? ($homepage = '&nbsp;') : ($homepage = a({-href=>"http://$homepage"},$homepage));
	...
	if ($logged_in) {
		...
		if ($method eq 'validate') {
			...
			if (scalar @errors == 0) {
				...
				$name = encode_entities($name, '<>');
				## I'm going to allow links in the group description for now, but
				## this might need to be reverted if it is abused
				#$description = encode_entities($description,'<>');
...
END
		...
		if (($check eq 'true') && ($data_valid == 1)) {
			...
			foreach my $member_email (keys %emails) {
				...
				print MAIL "<br>\n";
				print MAIL "VeloCal supported by <a href='http://www.stackhousemortgage.com'>www.stackhousemortgage.com</a><br>\n";
				print MAIL "<br>\n";
				...
				print MAIL "<br>\n";
				print MAIL "<hr>\n";
				print MAIL "Sponsored Links:<br>\n";
				print MAIL "<ul>\n";
				print MAIL "<li><a href='http://www.kansascyclist.com/podcast'>Kansas Cyclist Podcast</a>: Covering Bicycling News and Information from Arkansas, Colorado, Iowa, Kansas, Missouri, Nebraska, and Oklahoma\n\n";
				print MAIL "</ul>\n";
		...
		if ($authorized) {
			if ($confirmed == 1) {
				...
				print 
					...
					p(font({-color=>"red"},"WARNING") . ": You have selected to delete this event and remove it from everyone's calendars. You CANNOT remove the event from only your calendar.  Please carefully consider this action as it impacts all users of this group. If you would prefer to unsubscribe from this group and remove all future events for this group from your calendar, click " . a({-href=>"$script?a=unsubscribe&g=$group_id"},"here") . "."),
